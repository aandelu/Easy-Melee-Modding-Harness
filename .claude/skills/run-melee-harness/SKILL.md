...
## Run (human path)
There is no separate "human launch" — the harness *is* how you launch. To just play offline, run `play_wavedash_offline.py` (above) and click the Dolphin window. Online testing goes through `Harness.enter_online(peer=Peer())` — read WORKFLOW.md's online chapter first (different, desync-sensitive rules). `play_d2.py` is **offline-only**: its hook is consumer-side and desyncs netplay.
...
- **dme re-attach from a new process gives no-hook / torn reads.** Launch + hook + observe in ONE process. (Cost a detour this session; `play_wavedash_monitor.py` is the in-process pattern — its failed re-attach predecessor was deleted 2026-07-24, git history has it.)
