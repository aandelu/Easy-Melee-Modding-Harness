...
- **PadRead hook at `0x803775B8`** is the working input-injection path (`candidate_d2.py`, `auto_lcancel/auto_lcancel.py`). On entry: `r24` = 0-indexed port, `r25` = pad struct ptr. Pad struct layout: **16-bit buttons** at `0(r25)` (A=0x100, B=0x200, X=0x400, Y=0x800, L=0x40, R=0x20, Z=0x10), **stickX** signed byte at `2(r25)`, **stickY** signed byte at `3(r25)`. This is a different layout from the 32-bit `CONTROLLER_DIGITAL` the address sheet documents.
...
- **Trust empirical measurements over CSV descriptions for fields whose value you can directly observe.** `Char Data + 0x2354` ("Landing Lag Divisor") doesn't change on Fox L-cancels in this Slippi build; the real L-cancel observable is **landing-state duration** (15f → 7f on NAIR).
